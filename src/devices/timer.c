...
/** [Project 1] List of threads that are sleeping. */
...
/** Sleeps for approximately DURATION_TICKS (renamed in Project 1) timer ticks.
...
timer_sleep (int64_t duration_ticks)
{
  ...
  current->wakeup_time
    = duration_ticks + ticks; // Use static ticks since intr already disabled

  /* Insert sorted by wakeup_time to minimize iteration time in interrupt
   * handler. */
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  if (woke_up) {
    thread_preempt_if_needed (); // [Project 1 Task 2.1] Preempt if needed.
  }
  /* [Project 1] End */
}
...
/** [Project 1] Comparison function for sorting sleep list by wakeup_time.
